...
Если по кайкой либо причине не открывается можно попробовать `docker_start_EN_.bat`.

Если ничего не работает запустить через PS.

- Перейти в папку TrueCodeCurrencyService
- Запустить PS
- Выполнить команду docker-compose up -d

### 3. Остановка сервисов

- `docker-stop.bat` - остановка всех контейнеров (или `docker-stop-EN.bat` если не открывается)
...
| GET | `/favorites-finance/{userId}` | Получить полную информацию о всех избранных валютах пользователя по идентификатору пользователя | ❌ |
